...
const materials = [];
let particleGroup; // ✅ group to rotate everything
...
function init() {
	...
	const sprite1 = textureLoader.load( 'pngs/IMG_1495.png', assignSRGB );
    ...
    const scaleFactor = 3; // adjust to taste
    parameters = [
        [[ 1.0, 0.2, 0.5 ], sprite2, 40 * scaleFactor ],
        [[ 0.95, 0.1, 0.5 ], sprite3, 30 * scaleFactor ],
        [[ 0.90, 0.05, 0.5 ], sprite1, 20 * scaleFactor ],
        [[ 0.85, 0, 0.5 ], sprite5, 16 * scaleFactor ],
        [[ 0.80, 0, 0.5 ], sprite4, 10 * scaleFactor ]
    ];

	// ✅ Group for orbiting effect
	...
	for ( let i = 0; i < parameters.length; i ++ ) {
		...
		particleGroup.add( particles ); // ✅ add to group instead of scene
	}
...
function render() {
	...
	// ✅ orbit effect
	particleGroup.rotation.y = time;        // spin around Y axis
	particleGroup.rotation.x = time * 0.25; // slight tilt for 3D depth
